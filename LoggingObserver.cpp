...
Subject::Subject(const Subject& other) : observers(new std::vector<Observer*>(*other.observers)) {}
Subject& Subject::operator=(const Subject& other) {
    if(this != &other) *observers = *other.observers;
    return *this;
}
...
void Subject::notify() {
    ...
    if(!src) return;
    for(auto o : *observers) o->update(src);
}
...
LogObserver::~LogObserver() { if(logStream){ logStream->close(); delete logStream; } }
